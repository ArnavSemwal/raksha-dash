...
class RakshaUrineAnalysisScreen extends StatelessWidget {
  ...
  const RakshaUrineAnalysisScreen({
    super.key,
    this.status = UrineScanStatus.abnormal,
  ...
  Widget _buildInstructionText() {
    return const Padding(
      ...
      child: Text(
        'Dip test strip in sample for 2 seconds\nand place on reader.',
  ...
  Widget _buildBottomSection() {
    return Column(
      ...
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        if (status == UrineScanStatus.initial) ...[
          SizedBox(
            height: 54,
            child: ElevatedButton(
              onPressed: onStartScanPressed,
        ...
        ] else if (status == UrineScanStatus.abnormal) ...[
          _buildStatusBanner(
            bgColor: abnormalBg,
          ...
          _buildRetakeLink(),
          const SizedBox(height: 12),
        ] else if (status == UrineScanStatus.clean) ...[
          _buildStatusBanner(
            bgColor: cleanBg,
            borderColor: cleanBorder,
            textColor: cleanText,
            icon: Icons.science_outlined,
            text: 'URINE ANALYSIS: CLEAN',
          ),
          const SizedBox(height: 8),
          _buildRetakeLink(),
          const SizedBox(height: 12),
...
class _TestStripPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.4, h * 0.2, w * 0.2, h * 0.6),
      linePaint,
    );
  }
